...
async def test_race_condition_concurrent_checkouts(client, seed_data):
    ...
    logger.info(f"[TEST] Resultados Finais: Sucessos={success_count}, Conflitos={conflict_count}")
...
async def test_full_atribuicao_cycle(client, seed_data):
    ...
    async with test_session_maker() as session:
        result = await session.execute(select(Atribuicao).filter(Atribuicao.checkin_time.is_(None)))
